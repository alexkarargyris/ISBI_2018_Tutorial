...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.window_center and self.window_width:
            if np.random.random() < 0.3:
                x = self.apply_windowing(x, self.window_center, self.window_width)
    ...
    def apply_windowing(self, x, window_center, window_width):

        if np.isscalar(window_center):
            center = window_center
        else:
            center = np.random.uniform(window_center[0], window_center[1])

        if np.isscalar(window_width):
            width = window_width
        else:
            width = np.random.uniform(window_width[0], window_width[1])

        input_min = center - 0.5 * width
        input_max = center + 0.5 * width
        return rescale_intensity(x, in_range=(input_min, input_max), out_range=(0, 255))
